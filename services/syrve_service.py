    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to authenticate with Syrve API: {response.status_code} - {response.text}")
                return False
    ...
    async def create_invoice(self, invoice_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code in (200, 201):
                ...
                logger.error(f"Failed to create invoice in Syrve: {response.status_code} - {response.text}")
                return None
