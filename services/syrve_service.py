...
async def send_invoice_to_syrve(token, invoice_data):
    ...
    ET.SubElement(root, "documentNumber").text = invoice_data.get("number", f"INV-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}")
    ...
    # Добавляем дату
    ET.SubElement(root, "dateIncoming").text = invoice_data.get("date", datetime.datetime.now().strftime("%d.%m.%Y"))
