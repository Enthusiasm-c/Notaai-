...
from utils.learning import get_product_id_from_mapping, save_learned_mapping
from utils.match import get_product_by_id, match, save_product

# Set up logging
logger = logging.getLogger(__name__)


async def handle_item_matching(
    update: Update, context: ContextTypes.DEFAULT_TYPE, item: dict
) -> dict:
    """
    ...
    if product_id:
        ...
        if product:
            ...
            if score > 0.9:
                ...
                logger.info(
                    f"Automatically saved mapping for '{item_name}': {product_id} (score: {score:.2f})"
                )
...
async def send_item_selection(
    update: Update, context: ContextTypes.DEFAULT_TYPE, item_name: str, candidates: list
) -> None:
    """
    ...
    for product in candidates[:5]:
        ...
        callback_data = json.dumps(
            {"action": "select_item", "item_name": item_name, "product_id": product_id}
        )

        # Add button to keyboard
        keyboard.append(
            [InlineKeyboardButton(f"{product_name} ({score:.2f})", callback_data=callback_data)]
        )

    # Add "Not in list" option
    callback_data = json.dumps({"action": "not_in_list", "item_name": item_name})
    keyboard.append(
        [InlineKeyboardButton("Нет в списке / Добавить новый", callback_data=callback_data)]
    )
...
async def handle_new_product(
    ...
    context.user_data["adding_product"] = {"item_name": item_name, "step": "request_id"}
...
async def handle_product_id_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_chat.send_message(
        f"ID товара: <b>{product_id}</b>\n\n" "Теперь введите название товара:", parse_mode="HTML"
    )
