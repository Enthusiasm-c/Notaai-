...
from config import WAIT_PHOTO, CONFIRMATION, user_data
from utils.error_handling import log_error, save_error_image
from utils.invoice_processing import match_invoice_items, apply_unit_conversions, format_invoice_data
...
async def process_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info(f"OCR processing completed: {len(invoice_data.get('lines', []))} lines extracted")
        ...
        unmatched_items = [item for item in matched_data.get('lines', []) if item.get('product_id') is None]
