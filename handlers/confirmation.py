...
async def handle_invoice_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE, data: dict) -> None:
    """
    ...
    if not invoice_data:
        ...
        await query.edit_message_text("Извините, данные накладной не найдены. Пожалуйста, загрузите накладную заново.")
        return
    ...
    if not syrve_service:
        ...
        await query.edit_message_text("Извините, сервис Syrve недоступен. Пожалуйста, обратитесь к администратору.")
        return
...
async def handle_invoice_rejection(update: Update, context: ContextTypes.DEFAULT_TYPE, data: dict) -> None:
    """
...
async def handle_invoice_edit(update: Update, context: ContextTypes.DEFAULT_TYPE, data: dict) -> None:
    """
...
def create_new_invoice_keyboard() -> InlineKeyboardMarkup:
    ...
    """
    keyboard = [
        [InlineKeyboardButton("Загрузить новую накладную 📄", callback_data="new_invoice")]
    ]
    
    return InlineKeyboardMarkup(keyboard)

async def send_invoice_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE, invoice_data: dict) -> None:
    """
    ...
    await update.effective_chat.send_message(
        text=message,
        parse_mode="HTML",
        reply_markup=create_confirm_reject_keyboard(invoice_id)
    )
