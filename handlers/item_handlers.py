...
from config import (
    WAIT_PHOTO, CONFIRMATION, EDIT_ITEM, ADD_NEW_ITEM, 
    SET_CONVERSION, SELECT_EDIT_ITEM, user_data
)
from utils.error_handling import log_error
from utils.invoice_processing import format_invoice_data, check_product_exists
...
async def display_item_selection(query, user_id):
    ...
    for i, line in enumerate(matched_data.get('lines', [])):
        ...
        keyboard.append([InlineKeyboardButton(f"Edit item #{line_num}", callback_data=f"edit_item:{i}")])
...
async def handle_item_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if query.data == "back_to_main":
            ...
            unmatched_items = [item for item in user_data[user_id]['matched_data'].get('lines', []) 
...
async def display_item_edit_options(query, user_id, item_index):
    ...
    unmatched_indices = []
    if is_unrecognized:
        unmatched_indices = [i for i, l in enumerate(matched_data.get('lines', [])) if l.get('product_id') is None]
    ...
    keyboard.append([InlineKeyboardButton("Enter correct item", callback_data=f"manual_match:{item_index}")])
    keyboard.append([InlineKeyboardButton("Add as new item", callback_data=f"add_new:{item_index}")])
    
    # Для неопознанных товаров добавляем кнопку перехода к следующему
    if is_unrecognized and len(unmatched_indices) > 1:
        next_idx = unmatched_indices[(unmatched_indices.index(item_index) + 1) % len(unmatched_indices)]
        keyboard.append([InlineKeyboardButton("Skip to next unrecognized", callback_data=f"next_unmatched:{next_idx}")])
    
    # Кнопка для конвертации единиц измерения
    keyboard.append([InlineKeyboardButton("Set unit conversion", callback_data=f"set_conversion:{item_index}")])
...
async def handle_item_edit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Добавление товара как нового
            ...
            keyboard = [
                [
                    InlineKeyboardButton("Yes, add as new", callback_data=f"confirm_add_new:{item_index}"),
            ...
            if next_unmatched is not None:
                ...
                await query.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("Review Final Invoice", callback_data="final_preview")],
                        [InlineKeyboardButton("Cancel", callback_data="cancel_process")]
                    ])
                )
                return CONFIRMATION
        ...
        # Возврат к предыдущему шагу редактирования
            ...
            if not user_data[user_id].get('edit_history'):
                await query.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("Continue Editing", callback_data=f"back_to_edit:{current_index}")]
                    ])
                )
                return EDIT_ITEM
        ...
        # Возврат к основному экрану
            ...
            # Проверяем наличие неопознанных товаров
            unmatched_items = [item for item in matched_data.get('lines', []) 
                              if item.get('product_id') is None]
...
async def handle_manual_item_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if exists and product_id:
            ...
            keyboard = [
                [
                    InlineKeyboardButton("Yes, add as new", callback_data=f"confirm_manual_new:{item_index}:{entered_name}"),
                    InlineKeyboardButton("No, try again", callback_data=f"retry_manual:{item_index}")
                ]
            ]
        ...
        if next_unmatched is not None:
            ...
            keyboard = [
                [InlineKeyboardButton("Enter correct item", callback_data=f"manual_match:{next_unmatched}")],
                [InlineKeyboardButton("Add as new item", callback_data=f"add_new:{next_unmatched}")],
                [InlineKeyboardButton("Set unit conversion", callback_data=f"set_conversion:{next_unmatched}")]
            ]
            
            # Добавляем кнопку возврата к предыдущему шагу
            if user_data[user_id].get('edit_history'):
                keyboard.append([InlineKeyboardButton("Previous Step", callback_data="previous_step")])
    ...
    try:
        ...
        if query.data.startswith("confirm_manual_new:"):
            ...
            if len(parts) > 2:
                ...
                if 'awaiting_item_name' not in context.user_data or 'entered_name' not in context.user_data['awaiting_item_name']:
    ...
    try:
        ...
        if current_step == 'target_unit':
            ...
            try:
                ...
                save_unit_conversion(product_id, product_name, source_unit, target_unit, conversion_factor)
                ...
                # Проверяем наличие неопознанных товаров
                unmatched_items = [i for i, line in enumerate(matched_data.get('lines', [])) 
                                   if line.get('product_id') is None]
                
                if unmatched_items:
                    keyboard.append([
                        InlineKeyboardButton("Continue with unrecognized items", 
